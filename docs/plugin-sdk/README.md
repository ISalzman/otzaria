...
### אימות מקומי — לפני הדחיפה

ה-CI הוא גם שלב הפרסום: כשהאימות נכשל, הגרסה פשוט לא עולה לחנות. כדי לגלות זאת לפני
הדחיפה ולא אחריה, אפשר להריץ בדיוק את אותו וולידטור מקומית, מתוך תיקיית התוסף:

```bash
npx --yes github:Otzaria/otzaria-plugin-validator#v1 . --publish false
```

אין מה להתקין ואין מה לעדכן: `npm` פותר את התג `v1` מחדש בכל הרצה ושומר במטמון לפי
ה-commit שנפתר. לכן כל שחרור של הוולידטור מגיע אליכם מיד בהרצה הבאה, ואי אפשר להיתקע
עם עותק ישן. `--publish false` מוודא שהרצה מקומית לעולם אינה נוגעת בחנות.

#### הרצה אוטומטית לפני כל קומיט (git hook)
...
הפעלה חד-פעמית, כך שגם כל מי שישכפל את הריפו יקבל את ה-hook:
...
**הפסיקה נלקחת מהפלט של הוולידטור ולא מקוד היציאה של `npx`.** הוולידטור מדפיס
`OUTPUT passed=true|false` גם בהרצה מקומית, וזו העדות האמינה לכך שהוא באמת רץ:
...
תקלת תשתית (למשל `UNABLE_TO_GET_ISSUER_CERT_LOCALLY` מאחורי proxy ממסר-TLS). hook שנשען
עליו היה חוסם קומיטים בגלל תעודה ולא בגלל התוסף. בנוסף `npx` מול ספק git חייב לפתור את
הרפרנס מול GitHub, ולכן **אינו יכול לרוץ ללא רשת** — גם `--offline` ו-`--prefer-offline` יוצאים
בקוד `128` — וברשת גרועה הוא עלול להיתקע, ומכאן ה-`timeout`.
הכלל: **כשל אימות חוסם, כשל תשתית לא.**

לעקיפה חד-פעמית מכוונת: `git commit --no-verify`.

...
// true — נשלח | false — המשתמש ביטל בדיאלוג
```
...
- `reporterEmail` (אופציונלי) - כתובת המשתמש לחזרה; אם לא נמסרה, נלקחת כתובת השולח השמורה בהגדרות דיווח השגיאות (ואם גם היא ריקה — לא נשלחת כלל)

**דיאלוג אישור:** לפני השליחה מוצג למשתמש דיאלוג עם שם התוסף, יעד הדיווח ותצוגה מקדימה של הטקסט. אין דרך לעקוף אותו.

**החזרה:** `true` אם הדיווח נשלח, `false` אם המשתמש ביטל.

**שגיאות אפשריות:** `error.internal` — `details` חסר או ריק, כשל רשת, timeout, או תשובת שרת שאינה 2xx (כולל חסימת קצב). אין תור המתנה offline: כשל מוחזר לתוסף כדי שיודיע למשתמש ויאפשר ניסיון חוזר.

...
class CrossWindowTabDrag {
  ...
  /// כמה הסמן צריך לעלות (בפיקסלים פיזיים) מהנקודה הנמוכה ביותר שהגרירה
  /// הגיעה אליה, כדי שההתקרבות לראש הצג תיחשב יציאה מעלה.
  ///
  /// ## למה נדרשת נסיעה מעלה ולא די בהימצאות באזור
  ///
  ...
  /// ב-`(582,0)` לא. לכן המסירה מוקדמת מכאן והלאה.
  ///
  /// ⚠️ והנסיעה היא מה שמפריד אותה מסידור כרטיסיות. סידור הוא מחווה
  /// אופקית שאינה עולה, ובלי התנאי הזה כל גרירה שהתחילה ליד ראש המסך
  /// הייתה נמסרת מיד — כלומר סידור פנימי היה נעלם מהתוכנה.
  static const int _upwardTravel = 24;
  ...
  Future<void> _poll() async {
    ...
    if (target.isSelf) {
      ...
      // מה שמשאיר ללולאת ההזזה נסיעה שה-shell יראה. ראו [_upwardTravel].
      //
      // ⚠️ ורק כשמוק החלון כבר בידי הנייטיב — ראו [_snapshotSent].
      if (target.approachingTop &&
          deepest - target.y >= _upwardTravel &&
          _snapshotSent) {
        if (!_transferable) {
          _rejectTransfer();
          return;
  ...
  Future<void> _captureDragOrigin() async {
    final target = await _service.windowAtCursor();
    // הגרירה הסתיימה בינתיים, או שפעימה כבר מדדה נקודה נמוכה יותר.
    if (target == null || _draggedTitle == null) return;

...
void main() {
  group('resolveLeftPaneSearchFocus', () {
    test('פתיחת הפאנל (סגור→פתוח) מבקשת פוקוס פעם אחת', () {
      final result =
          resolveLeftPaneSearchFocus(showLeftPane: true, wasShown: false);
    ...
    test('פאנל סגור — לא מבקש פוקוס ומאפס את מצב "הוצג"', () {
      final result =
          resolveLeftPaneSearchFocus(showLeftPane: false, wasShown: true);
      expect(result.shouldFocus, isFalse);
      expect(result.wasShownNext, isFalse);
    });

    test('רגרסיה: rebuild-ים חוזרים בזמן גלילה (פאנל פתוח) לא מבקשים פוקוס שוב',
        () {
      ...
      var result =
          resolveLeftPaneSearchFocus(showLeftPane: true, wasShown: wasShown);
    ...
    test('סגירה ופתיחה מחדש מבקשות פוקוס שוב (פוקוס בכל פתיחה)', () {
      ...
      // סגירה
      result =
          resolveLeftPaneSearchFocus(showLeftPane: false, wasShown: wasShown);
      expect(result.shouldFocus, isFalse);
      wasShown = result.wasShownNext;

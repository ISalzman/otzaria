...
void main() {
  ...
  group('ExpandableCard', () {
    ...
    testWidgets('כשisExpanded משתנה ל-false, sizeFactor חוזר ל-0 אחרי אנימציה',
        (tester) async {
      bool expanded = true;
      late StateSetter setter;
    ...
    testWidgets('margin מוחל כ-Padding סביב הכרטיס', (tester) async {
      ...
      final padding = tester
          ...
          .firstWhere((p) => p.padding == testMargin, orElse: () => throw 'לא נמצא Padding עם margin');
    ...
    testWidgets('wrapInCard:true — עוטף ב-Material עם ClipBehavior', (tester) async {
      final key = GlobalKey();
      await tester.pumpWidget(_wrap(
        ExpandableCard(
          key: key,
    ...
    testWidgets('wrapInCard:false — אין Material חיצוני עם clipBehavior', (tester) async {
      final key = GlobalKey();
      await tester.pumpWidget(_wrap(
        ExpandableCard(
          key: key,

...
void main() {
  group('בולד אמיתי — מסלול מהיר (SimpleInlineHtml → Text.rich)', () {
    testWidgets('גופן משתנה (Rubik) → הספאן המודגש מכיל FontVariation wght 700',
        (tester) async {
    ...
    testWidgets(
      ...
      await tester.pumpWidget(_wrap(const SmartTextWidget(
        text: '<b>מודגש</b> רגיל',
        settings: RenderSettings(fontSize: 20, fontFamily: 'FrankRuhlCLM'),
      )));
      ...
      final boldStyle = bold.single.style!;
      expect(boldStyle.fontWeight, FontWeight.bold);
      expect(boldStyle.fontVariations, isNull,
  ...
  group('בולד אמיתי — מסלול fwfh (HtmlWidget + WidgetFactory)', () {
    testWidgets(
      ...
      final withVariation = allBold.where((s) =>
          s.style?.fontVariations != null &&
          s.style!.fontVariations!.contains(const FontVariation('wght', 700)));
  ...
  group('בולד אמיתי — ContinuousReadingParagraph', () {
    testWidgets('<b> עם גופן משתנה → הספאן המודגש מכיל FontVariation wght 700',
        (tester) async {
      ...
      final bold = _spansContaining(rich.text, 'מודגש');
      expect(bold, isNotEmpty);
      final withVariation = bold.where((s) =>
          s.style?.fontVariations != null &&
          s.style!.fontVariations!.contains(const FontVariation('wght', 700)));
      expect(withVariation, isNotEmpty);
      expect(withVariation.first.style!.fontWeight, FontWeight.bold);
    ...
    testWidgets('<b> עם גופן סטטי → fontWeight.bold בלי fontVariations',
      ...
      final bold = _spansContaining(rich.text, 'מודגש');
      expect(bold, isNotEmpty);
      final boldWithWeight =
          bold.where((s) => s.style?.fontWeight == FontWeight.bold);
